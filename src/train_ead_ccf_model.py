...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train corporate EAD/CCF model.")
    ...
    parser.add_argument("--table", default="mart.ead_ccf_modeling_sample")
...
def clip_ccf_values(values: pd.Series) -> pd.Series:
    numeric = pd.to_numeric(values, errors="coerce")
    return numeric.clip(lower=0, upper=1.5)
...
def get_ead_feature_columns(df: pd.DataFrame) -> list[str]:
...
def make_ead_time_split(
    df: pd.DataFrame,
    date_col: str = DATE_COL,
    target_col: str = TARGET_CCF,
    train_size: float = 0.70,
    valid_size: float = 0.15,
) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    data = df.copy()
    data[date_col] = pd.to_datetime(data[date_col], errors="coerce")
    data[target_col] = clip_ccf_values(data[target_col])
        ...
        [
            {
                ...
                "avg_ccf": train_df[target_col].mean(),
            },
            {
                "split": "valid",
                "rows": len(valid_df),
                ...
                "avg_ccf": valid_df[target_col].mean(),
            },
            {
                "split": "oot",
                "rows": len(oot_df),
                ...
                "avg_ccf": oot_df[target_col].mean(),
            },
        ]
    )
...
def predict_ccf(model: Pipeline, X: pd.DataFrame) -> np.ndarray:
    predictions = model.predict(X)
    return np.clip(predictions, 0, 1.5)


def build_ead_by_product_report(
    df: pd.DataFrame,
    actual_col: str,
    pred_col: str,
) -> pd.DataFrame:
    report = (
        ...
        .agg(
            ...
            avg_actual_ccf=(actual_col, "mean"),
            avg_predicted_ccf=(pred_col, "mean"),
    ...
    report["prediction_error"] = report["avg_predicted_ccf"] - report["avg_actual_ccf"]

    return report.sort_values("facilities", ascending=False)


def build_ead_calibration_by_decile(
    df: pd.DataFrame,
    actual_col: str,
    pred_col: str,
    n_bins: int = 5,
    ...
    report = (
        data.groupby("ccf_decile", as_index=False)
        .agg(
            facilities=("facility_id", "size"),
            avg_actual_ccf=(actual_col, "mean"),
            avg_predicted_ccf=(pred_col, "mean"),
...
def train_and_evaluate(df: pd.DataFrame, include_xgboost: bool) -> None:
    ...
    models_dir = MODELS_DIR / "ead_ccf"
    models_dir.mkdir(parents=True, exist_ok=True)

    df[TARGET_CCF] = clip_ccf_values(df[TARGET_CCF])
    df = coerce_numeric_like_columns(df, protected_columns=ID_AND_TARGET_COLUMNS)

    feature_columns = get_ead_feature_columns(df)
    ...
    pd.DataFrame({"feature": feature_columns}).to_csv(
        REPORTS_DIR / "ead_ccf_feature_columns.csv",
        index=False,
    )

    train_df, valid_df, oot_df, split_summary = make_ead_time_split(df)
    split_summary.to_csv(REPORTS_DIR / "ead_ccf_time_split_summary.csv", index=False)

    print("EAD/CCF time split summary")
    print(split_summary)
    print()

    X_train = train_df[feature_columns]
    y_train = train_df[TARGET_CCF].astype(float)
    ...
    oot_predictions = oot_df[
        ["facility_id", "company_id", DATE_COL, "product_type", TARGET_CCF, WEIGHT_COL]
    ].copy()
    ...
    for model_name, model in models.items():
        ...
        for split_name, split_df in splits.items():
            ...
            if split_name == "oot":
                oot_predictions[f"ccf_{model_name}"] = y_pred

    metrics_df = pd.DataFrame(metrics_rows)
    metrics_df = metrics_df.sort_values(["split", "weighted_mae"], ascending=[True, True])
    metrics_df.to_csv(REPORTS_DIR / "ead_ccf_model_metrics.csv", index=False)
    ...
    best_ccf_column = f"ccf_{best_model_name}"

    ead_by_product = build_ead_by_product_report(
        oot_predictions,
        actual_col=TARGET_CCF,
        pred_col=best_ccf_column,
    )
    ead_by_product.to_csv(REPORTS_DIR / "ead_ccf_by_product.csv", index=False)

    calibration = build_ead_calibration_by_decile(
        oot_predictions,
        actual_col=TARGET_CCF,
        pred_col=best_ccf_column,
        n_bins=5,
    )
    calibration.to_csv(REPORTS_DIR / "ead_ccf_calibration_by_decile.csv", index=False)

    oot_predictions.to_csv(REPORTS_DIR / "ead_ccf_oot_predictions.csv", index=False)
    pd.DataFrame([best_row]).to_csv(REPORTS_DIR / "ead_ccf_best_model_summary.csv", index=False)

    print("EAD/CCF model metrics")
    print(metrics_df)
    print()
    print(f"Best OOT model: {best_model_name}")
    print()
    print("EAD/CCF calibration by decile")
    print(calibration)
    print()
    print(f"Reports saved to: {REPORTS_DIR}")
    print(f"EAD/CCF models saved to: {models_dir}")
...
def main() -> None:
    ...
    df[TARGET_CCF] = clip_ccf_values(df[TARGET_CCF])
    df = df.dropna(subset=[DATE_COL, TARGET_CCF]).copy()
    ...
    print(f"Average CCF: {df[TARGET_CCF].mean():.4f}")
