DROP TABLE IF EXISTS mart.lgd_modeling_sample;

CREATE TABLE mart.lgd_modeling_sample AS
WITH base AS (
    SELECT
        frm.facility_id,
        frm.company_id,
        ...
        COALESCE(frm.total_recovery_amount, 0) AS total_recovery_amount,
        COALESCE(frm.total_collection_cost, 0) AS total_collection_cost,
        COALESCE(frm.recovery_events_count, 0) AS recovery_events_count,

        GREATEST(
            COALESCE(frm.total_recovery_amount, 0) - COALESCE(frm.total_collection_cost, 0),
            0
        ) AS net_recovery_amount,
...
SELECT
    *,
    LEAST(
        ...
        GREATEST(
            0.0,
            1.0 - net_recovery_amount / NULLIF(ead_at_default, 0)
        )
    ) AS realized_lgd
FROM base;

CREATE INDEX IF NOT EXISTS idx_lgd_modeling_sample_facility
    ON mart.lgd_modeling_sample (facility_id);

CREATE INDEX IF NOT EXISTS idx_lgd_modeling_sample_default_date
    ON mart.lgd_modeling_sample (default_date);

CREATE INDEX IF NOT EXISTS idx_lgd_modeling_sample_industry
    ON mart.lgd_modeling_sample (industry);
